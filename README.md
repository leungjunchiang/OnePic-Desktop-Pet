...
应用和安装文件名为“Lili”，宠物本人仍叫“六毛”。`v0.12.0` 使用仓库所有者新提供的 46 张独立动作图，并为完整动作加入平滑交叉淡化；不登录时仍是完整的纯离线桌宠。动作统一保留六束紧密红色头发、蓝色发梢、可见小嘴、蓝色锯齿脸、黄色连体衣和粉色爱心。
...
- 心情系统只给正向反馈，没有没工作就挨饿、生病或掉好感的惩罚；原有 14 套服装保留为长期收藏；
- 可用邮箱登录“搭子与自习室”，通过 8 位搭子码互加好友，查看在线、工作/休息状态和可选的今日累计；
- 可创建或通过房间码加入私人自习室，向好友派六毛串门；对方接受后双方桌面各自在本地显示双六毛共同工作画面；
...
源码维护在 [leungjunchiang/OnePic-Desktop-Pet](https://github.com/leungjunchiang/OnePic-Desktop-Pet)。`v0.12.0` 提供 Lili 的 Windows ZIP，并由发布流程生成 Apple 芯片 Mac DMG、Intel Mac DMG及各自的 SHA-256 校验文件；旧版本仍保留在 Releases 页面。
