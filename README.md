...
应用和安装文件名为“Lili”，宠物本人仍叫“六毛”。`v0.7.0` 默认站立高度缩小为 160，并提供 100–360 像素连续滑块；六束紧密红色头发、蓝色发梢与锯齿脸、黄色连体衣和粉色爱心始终保持一致。
...
- 双击六毛可打开快捷口袋，不依赖 Windows 托盘或 Mac 菜单栏；
- 能按前台应用类别切换电脑、写字、看书、耳机等动作；只识别应用类别，不读取窗口标题或文档内容；
- 可主动选择弹吉他、打鼓，并由用户点击后打开网易云音乐或 QQ 音乐的陈楚生歌曲正版搜索页；
...
程序没有打包陈楚生歌词或音频。它只保存歌曲标题与原创化的意象短句；音乐按钮打开正版平台搜索页，实际播放仍由网易云音乐或 QQ 音乐及用户操作决定。
...
源码维护在 [leungjunchiang/OnePic-Desktop-Pet](https://github.com/leungjunchiang/OnePic-Desktop-Pet)。`v0.7.0` 提供 Lili 的 Windows ZIP、Apple 芯片 Mac DMG、Intel Mac DMG 及各自的 SHA-256 校验文件；旧版本仍保留在 Releases 页面。
