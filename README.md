...
自习室网络通过 `SocialClient` 的后端接口解耦：默认使用现有 Supabase；部署轻量中转服务后，可在 `config/social_backend.json` 或环境变量 `LILI_SOCIAL_API_BASE_URL` 设置 `social_api_base_url`，客户端将改走 `/auth/*`、`/presence/*`、`/dashboard`、`/buddies/*`、`/rooms/*` 与 `/visits/*` 等稳定接口。客户端不包含 service-role key。
...
```powershell
.\scripts\publish_release.ps1 -Version 0.21.8 -NotesFile .\release-notes.md
```
