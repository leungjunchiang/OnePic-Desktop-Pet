...
自习室网络通过 `SocialClient` 的后端接口解耦：当前发布配置默认使用已部署的 Supabase Edge Function `lili-social-relay-v2`，地址为 `https://zkgctfntrioffpifiggk.supabase.co/functions/v1/lili-social-relay-v2`；仓库同时提供受限的 Cloudflare Worker 中转服务源码，位于 [`relay/`](relay/)。客户端将改走 `/auth/*`、`/presence/*`、`/dashboard`、`/buddies/*`、`/rooms/*` 与 `/visits/*` 等稳定接口。客户端不包含 service-role key。当前桌面端采用后台短轮询：正常网络下约 10 秒同步一次，切换房间和互动会立即刷新；首页提供“检测自习室网络”，会显示实际后端并区分 DNS、超时、拒绝连接、TLS、认证和服务器错误；若当前网络无法直连，客户端会快速失败、显示最近一次房间状态并自动重试，不会阻塞桌宠、计时或窗口。Edge Function 源码位于 [`supabase/functions/lili-social-relay/`](supabase/functions/lili-social-relay/)，Cloudflare 部署说明见 [`relay/README.md`](relay/README.md)。仓库不把未经中国大陆目标网络实测的公网地址标记为已验证。
