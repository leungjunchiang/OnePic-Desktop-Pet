...
应用和安装文件名为“Lili”，宠物本人仍叫“六毛”。`v0.9.0` 默认站立高度为 160，并提供 100–360 像素连续滑块；六束紧密红色头发、蓝色发梢与锯齿脸、黄色连体衣和粉色爱心始终保持一致。
...
- 可主动选择全幅耳机、吉他和鼓动作；设置页能选择本机网易云音乐或 QQ 音乐程序路径，点击随机歌曲后直接启动、自动搜索并尝试播放，未安装时才回退到正版网页；
...
- 每累计工作 1 小时解锁一套娃衣或配饰，共 14 套；新增梦想清扫员、飞行梦想家、甜梦抱抱和红毯小大人，最后一套为戴金色皇冠和披风的“荒野国王”；
...
公开程序没有打包陈楚生歌词或“巴布达”私人音频。它只保存歌曲标题与原创化的意象短句。你可以在“AI 与陪伴设置”选择本机音乐程序、本地巴布达音频和自己有权使用的歌词 TXT；路径与内容只在本机使用。Windows 客户端调用会在用户明确点击后短暂聚焦音乐软件，通过搜索框尝试播放第一条结果；不同客户端版本若没有响应，会保留搜索结果供用户点击。
...
源码维护在 [leungjunchiang/OnePic-Desktop-Pet](https://github.com/leungjunchiang/OnePic-Desktop-Pet)。`v0.9.0` 计划提供 Lili 的 Windows ZIP、Apple 芯片 Mac DMG、Intel Mac DMG 及各自的 SHA-256 校验文件；旧版本仍保留在 Releases 页面。
