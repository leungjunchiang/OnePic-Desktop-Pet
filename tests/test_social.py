...
def test_final_focus_sync_guard_cannot_reopen_legacy_duration_writes() -> None:
    root = Path(__file__).resolve().parents[1]
    migration = (
        root / "supabase" / "migrations" /
        "20260910163719_guard_legacy_focus_writes_after_frozen_floor.sql"
    ).read_text(encoding="utf-8")
    normalized = " ".join(migration.casefold().split())

    assert migration.count("create or replace function public.lili_sync_personal_state") == 2
    assert "client day/week duration scalars are ignored" in normalized
    assert "incoming client history is ignored" in normalized
    assert "public.lili_effective_focus_stats(current_user_id)" in normalized
    assert "public.lili_focus_union_seconds" in normalized
    function_definitions = normalized.split("do $verify$", 1)[0]
    assert "insert into public.lili_focus_daily" not in function_definitions
...
def test_peer_dashboard_projection_rewrites_visible_totals_to_effective_values():
    ...
    migration = (
        ...
        / "supabase"
        / "migrations"
        / "20260910150000_lili_dashboard_effective_buddy_totals.sql"
    ).read_text(encoding="utf-8")
    normalized = " ".join(migration.casefold().split())

    assert "create or replace function public.lili_mark_canonical_focus_totals" in normalized
    assert "target_user_id" in normalized
    assert "public.lili_effective_focus_today_seconds(target_user_id)" in normalized
    assert "public.lili_effective_focus_week_seconds(target_user_id)" in normalized
    assert "jsonb_set( item, '{today_seconds}'" in normalized
