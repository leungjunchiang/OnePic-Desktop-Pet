...
def test_owner_nickname_changes_social_identity_without_changing_pet_name() -> None:
    ...
    settings_dialog.owner_nickname.setText("å›¢å›¢")
    settings_dialog.apply()

    assert settings.owner_nickname == "å›¢å›¢"
    assert settings.pet_name == "å…­æ¯›"
    chat = ChatDialog(None, settings.pet_name)
    assert chat.windowTitle() == "å’Œå…­æ¯›èŠèŠ"
    assert chat.pet_title.text() == "å…­æ¯›çš„å°çº¸æ¡"
    assert chat.rename_button.text() == "ä¿®æ”¹ä¸»äººç§°å‘¼"
    assert "ä¸»äººç§°å‘¼" in chat.rename_button.toolTip() or "è‡ªä¹ å®¤" in chat.rename_button.toolTip()
    chat.set_pet_name("é˜¿æ¯›")
    assert chat.windowTitle() == "å’Œå…­æ¯›èŠèŠ"
    assert chat.input.placeholderText() == "è·Ÿå…­æ¯›è¯´ç‚¹ä»€ä¹ˆâ€¦â€¦"
...
def test_chat_rename_button_opens_visible_rename_flow(monkeypatch) -> None:
    ...
    assert window.settings.owner_nickname == "å›¢å­"
    assert window.settings.pet_name == "å…­æ¯›"
    assert window._chat_dialog.pet_title.text() == "å…­æ¯›çš„å°çº¸æ¡"
    assert window.windowTitle().endswith("Â· å…­æ¯›")
    window.close()
    window.deleteLater()
    app.processEvents()
...
def test_chat_connected_status_is_not_rendered_twice() -> None:
    dialog = ChatDialog()
    dialog.set_provider("codex", "connected", "Codex å·²è¿æ¥ã€‚")
    assert dialog.status_label.text() == "Codexï¼ˆä½¿ç”¨æœ¬æœºç™»å½•ï¼‰ Â· å·²è¿æ¥ï¼Œä¼˜å…ˆä½¿ç”¨ AI"
