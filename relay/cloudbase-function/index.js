...
const RPC_ALLOWLIST = new Set([
  ...
  "lili_sync_personal_state", "lili_sync_focus_history", "lili_sync_focus_segments", "lili_sync_focus_segments_delta", "lili_sync_focus_segments_delta_v2", "lili_focus_live_projection", "lili_focus_weekly_leaderboard", "lili_update_presence_context", "lili_upsert_focus_presence",
]);
...
async function handleRequest(event, env) {
  ...
  if (path === "/profile" && method === "PATCH") {
    ...
    for (const key of ["nickname", "owner_nickname"]) {
      ...
      // An omitted/empty identity field means “leave the durable profile
      // untouched”. Never turn a compatibility request into a destructive
