...
language sql stable security definer set search_path = '' as $$
  select jsonb_build_object(
    ...
    'room_people',coalesce((select jsonb_agg(distinct jsonb_build_object('user_id',p.user_id,'nickname',p.nickname,'outfit_key',p.outfit_key,
      'working',coalesce(f.working,false),'session_started_at',f.session_started_at,'today_seconds',case when p.show_exact_time then coalesce(f.today_seconds,0) else null end,
      'online',coalesce(f.last_seen>now()-interval '2 minutes',false)))
    ...
    'active_visits',coalesce((select jsonb_agg(jsonb_build_object('id',v.id,'peer_id',p.user_id,'nickname',p.nickname,'outfit_key',p.outfit_key,
      'working',coalesce(f.working,false),'session_started_at',f.session_started_at,'today_seconds',case when p.show_exact_time then coalesce(f.today_seconds,0) else null end))
