-- 荒野王国富豪榜：新用户默认参评，旧用户只迁移“从未主动设置”的记录。
-- preference_set=true 的用户代表明确选择过，绝不能被本迁移覆盖。

alter table public.lili_profiles
  alter column wealth_leaderboard_enabled set default true;

update public.lili_profiles
set wealth_leaderboard_enabled = true
where not wealth_leaderboard_preference_set;

-- 保留“仅已接受搭子可见”与“按本月创收排名”的规则。
