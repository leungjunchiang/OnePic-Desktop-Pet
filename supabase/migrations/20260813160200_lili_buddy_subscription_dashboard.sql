-- Include the optional buddy subscription state in the existing dashboard.
...
language sql stable security definer set search_path = '' as $$
  select jsonb_build_object(
    ...
    'me_presence',coalesce((select jsonb_build_object(
      ...
      'status',case when f.last_seen<=now()-interval '2 minutes' then 'offline' when f.working then 'focus' else 'rest' end,
      ...
      'online',f.last_seen>now()-interval '2 minutes','last_seen',f.last_seen
    ) from public.lili_focus_presence f where f.user_id=(select auth.uid())),
      jsonb_build_object('user_id',(select auth.uid()),'working',false,'status','idle','session_seconds',0,'today_seconds',0,'online',false)),
    'requests',coalesce((select jsonb_agg(jsonb_build_object('id',l.id,'nickname',p.nickname,'created_at',l.created_at))
      ...
      where l.addressee_id=(select auth.uid()) and l.status='pending'),'[]'::jsonb),
    'buddies',coalesce((select jsonb_agg(jsonb_build_object(
      ...
      'online',coalesce(f.last_seen>now()-interval '2 minutes',false),'is_self',false,
      'subscribed',coalesce((select s.on_focus_start or s.on_focus_end from public.lili_buddy_subscriptions s
        where s.subscriber_id=(select auth.uid()) and s.buddy_id=p.user_id and not s.muted),false)))
    ...
    'visits',coalesce((select jsonb_agg(jsonb_build_object('id',v.id,'sender_id',v.sender_id,'nickname',p.nickname,'kind',v.kind,'created_at',v.created_at))
      ...
      where v.receiver_id=(select auth.uid()) and v.status='pending' and v.expires_at>now()),'[]'::jsonb),
    'active_visits',coalesce((select jsonb_agg(jsonb_build_object(
      'id',v.id,'peer_id',p.user_id,'nickname',p.nickname,'outfit_key',coalesce(f.outfit_key,p.outfit_key),
      'working',case when f.working and f.last_seen>now()-interval '2 minutes' then true else false end,
      'status',case when f.last_seen is null or f.last_seen<=now()-interval '2 minutes' then 'offline' when f.working then 'focus' else 'rest' end,
      'session_started_at',f.session_started_at,
      'session_seconds',case when f.working and f.session_started_at is not null then greatest(0,floor(extract(epoch from (now()-f.session_started_at)))::int) else 0 end,
