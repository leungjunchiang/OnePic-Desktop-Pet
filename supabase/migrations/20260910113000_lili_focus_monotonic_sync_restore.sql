-- The ordered release deploy replays older function definitions.  Keep the
-- final sync writers monotonic so a restarted client with a shorter local
-- projection cannot reduce an already accepted server day or week.
