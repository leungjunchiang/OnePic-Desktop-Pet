-- 让自习室读取同一份最新专注状态：本人和搭子都返回 presence，
-- 并在 dashboard 中给出 focus/rest/offline 与本轮专注秒数。
-- 客户端仍不上传任务名、窗口标题或聊天内容。
...
language sql stable security definer set search_path = '' as $$
  select jsonb_build_object(
    ...
    'me_presence',coalesce((select jsonb_build_object(
      ...
      'today_seconds',coalesce(f.today_seconds,0),
      'outfit_key',coalesce(f.outfit_key,''),
      'room_id',f.room_id,
      'online',f.last_seen>now()-interval '2 minutes',
      'last_seen',f.last_seen
    ) from public.lili_focus_presence f where f.user_id=auth.uid()),
      jsonb_build_object('user_id',auth.uid(),'working',false,'status','idle','session_seconds',0,'today_seconds',0,'online',false)),
