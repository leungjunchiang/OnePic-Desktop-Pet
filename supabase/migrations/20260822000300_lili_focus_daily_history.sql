-- 按账号、按北京时间日期保存六毛的每日专注历史。
-- lili_profiles 继续保存今天/本周/终身汇总；本表负责跨电脑的逐日比较。

create table if not exists public.lili_focus_daily (
  ...
  constraint lili_focus_daily_seconds_check check (seconds between 0 and 86400)
);

create index if not exists lili_focus_daily_user_date_idx
  on public.lili_focus_daily (user_id, focus_date desc);

alter table public.lili_focus_daily enable row level security;

drop policy if exists "lili_focus_daily_owner_select" on public.lili_focus_daily;
create policy "lili_focus_daily_owner_select"
  on public.lili_focus_daily for select
  to authenticated
  using ((select auth.uid()) = user_id);

drop policy if exists "lili_focus_daily_owner_insert" on public.lili_focus_daily;
create policy "lili_focus_daily_owner_insert"
  on public.lili_focus_daily for insert
  to authenticated
  with check ((select auth.uid()) = user_id);

drop policy if exists "lili_focus_daily_owner_update" on public.lili_focus_daily;
create policy "lili_focus_daily_owner_update"
  on public.lili_focus_daily for update
  to authenticated
  using ((select auth.uid()) = user_id)
  with check ((select auth.uid()) = user_id);

-- The desktop uses the authenticated RPCs below so the table itself does not
-- need to be exposed through PostgREST.
revoke all on table public.lili_focus_daily from anon, authenticated;
