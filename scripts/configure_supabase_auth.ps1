...
$configSupabaseUrl = ""
$configRedirectUrl = ""
if ($null -ne $publicConfig) {
    if ($publicConfig.PSObject.Properties.Name -contains "supabase_url") {
        $configSupabaseUrl = [string]$publicConfig.supabase_url
    }
    if ([string]::IsNullOrWhiteSpace($configSupabaseUrl) -and
        ($publicConfig.PSObject.Properties.Name -contains "url")) {
        $configSupabaseUrl = [string]$publicConfig.url
    }
    if ($publicConfig.PSObject.Properties.Name -contains "email_redirect_to") {
        $configRedirectUrl = [string]$publicConfig.email_redirect_to
    }
}
...
} elseif (-not [string]::IsNullOrWhiteSpace($configSupabaseUrl)) {
    ([Uri]$configSupabaseUrl).Host.Split('.')[0]
} else {
    ""
}
...
} elseif (-not [string]::IsNullOrWhiteSpace($configRedirectUrl)) {
    $configRedirectUrl
} else {
    ""
}
...
if (-not [int]::TryParse($resolvedPort, [ref]$smtpPortNumber) -or
    $smtpPortNumber -lt 1 -or $smtpPortNumber -gt 65535) {
