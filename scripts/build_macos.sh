...
dmg_file="dist/OnePicDesktopPet-macOS-${release_arch}-unsigned.dmg"
...
test -d "dist/OnePicDesktopPet.app"

# The public build is intentionally unsigned because no Apple Developer ID is available.
codesign --force --deep --sign - "dist/OnePicDesktopPet.app"
rm -f "$dmg_file"
hdiutil create \
    -volname "OnePic Desktop Pet" \
    -srcfolder "dist/OnePicDesktopPet.app" \
    -ov \
    -format UDZO \
    "$dmg_file"
