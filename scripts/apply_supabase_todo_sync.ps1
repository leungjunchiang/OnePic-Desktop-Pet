...
[CmdletBinding()]
param(
    [string]$MigrationPath = "supabase/migrations/20260915120000_lili_todo_sync.sql",
...
# Todo has an independent deployment boundary.  This script sends exactly
# one Todo migration and never invokes the ordered focus/presence migration
# runner or a whole-account reset.
...
try {
    ...
    if ($null -ne $statusCode) {
        throw "Supabase Todo migration failed with HTTP $statusCode. The token needs database_write scope and the project must be reachable."
    }

    throw "Supabase Todo migration failed. Check the token scope, project ref, and network access."
}

Write-Host "Supabase Todo migration applied to project $projectRef."
