## v0.23.293 — 修复六毛闹钟完成后自定义歌曲仍循环播放

- 修复 Windows 闹钟关闭后，MCI 停止或关闭命令失败却被错误标记为完成，导致原生循环播放残留的问题；保留后端别名并在后台重试，直到确认关闭。
- MCI 尚未确认静音前不再启动 Qt 回退播放器，避免同一首自定义歌曲被两个音频后端叠加播放。
- 分离 GUI 状态锁与原生音频命令锁，闹钟按钮不会因 MCI 停止等待而卡住。

## v0.23.292 — 修复历史报告回补与跨午夜今日时长串日

- 打开工作报告时立即排队现有的后台同步，不阻塞窗口响应；服务器原始 FocusSegment 合并成功后，自动刷新已经打开的报告。
- 将账号级有界历史对账版本从 v1 升至 v2。已完成旧版对账的设备升级后会重新拉取最近 60 天的原始区间，按 Segment ID 幂等合并，不重置普通 delta cursor，也不覆盖服务器事实。
- 将本地精确历史恢复器升级到 v3，重新扫描仍保留的工作会话/生命周期区间；只有可验证的起止记录才会进入待上传队列，单纯累计秒数不会伪造为 Segment。
- 修复北京时间零点后“今日已工作”继续沿用前一天账号缓存并逐秒累加的问题；跨日时仅用已取得的区间及本机实时段重新投影当天时长，失败时退回当天本机值，不把昨天的总量带进今天。
- Windows 继续使用 3 秒非激活置顶 watchdog；保留 `SWP_NOACTIVATE`，不会为恢复窗口抢走 Word 等应用的键盘焦点。
- 未修改 FocusSegment 原始数据或其他账号同步数据。服务器没有原始记录的日期不会伪造工作时长。

## v0.23.291 — 修复 macOS 置顶恢复与今日工作小栏消失

- 修复 macOS 浮动层级恢复没有实际生效的问题：PyObjC 调用改用真实的无参数 `orderFrontRegardless()`，不再误查带尾部下划线的方法名。
- 切换到 Word 等其他应用时立即并在窗口层级稳定后再次无焦点恢复六毛与独立小栏；低频 watchdog 同时重新核对“今日已工作”小栏的业务可见状态。
- 保留真正全屏时的自动隐藏、用户手动隐藏和“显示今日工作”设置；恢复窗口时不调用激活或抢焦点接口。
- 未修改计时、FocusSegment、心跳或其他云端数据。

## v0.23.290 — 恢复工作报告中的可信历史数据

- 修复报告把多设备 FocusSegment 的合法重叠误判为旧版异常累计检查点的问题；现在按账号级区间并集统计，9 月 14 日起已有的可信数据会正常回到日报、月报、热力图和搭子统计。
- 只有没有稳定 FocusSegment 身份、确实符合旧版累计检查点形态的记录才会进入兼容隔离；原始记录保留，不清空、不重写服务器数据。
- 报告提示和日期悬浮说明改为明确区分“旧版重复检查点”和“日级兼容数据”，并增加多设备重叠事实不应被隔离的回归测试。
- 本版本不修改 Supabase schema、FocusSegment 原始事实、Presence、Heartbeat、Auth、Todo 或其他同步域。

## v0.23.283 — 修复双设备暂停后“今日已工作”继续增长

- 本机暂停或结束后，立即冻结此前缓存的跨设备 live interval；在服务器下一次确认前，不再把旧开放区间延长为新的工作时间。
- 服务器明确返回“没有工作设备”时，立即清空所有临时 live rows；两台电脑都暂停后，今日累计会停在同一个已封存的账号级总量。
- 为 live projection 缓存增加严格的 120 秒有效期。网络异常时保留最后显示值，但过期的开放区间不再继续计时。
- 未改动 FocusSegment 事实、历史统计、心跳阈值、认证、Todo 或其他同步域。

## v0.23.282 — 分离搭子实时状态与历史专注统计

- 搭子即使处于休息、离线或实时状态待确认，仍然显示服务器已经取得的今日与本周专注时间。
- presence 超时或缓存过期只影响实时状态，不再清空或隐藏已有的历史统计。
- 移除首页摘要中的“实时状态待确认”技术性提示，保留简洁的最近确认专注人数。
- 保留现有在线判定阈值、心跳协议、排序规则和其他同步域不变。

## v0.23.281 — 修复搭子排序与首屏状态加载

- “我的搭子”改为严格按实时状态、今日专注、本周专注、最后确认时间和稳定账号 ID 的字典序排列；正在专注、在线休息、黄灯/离线的优先级保持明确且稳定。
- 首次进入自习室时区分 `unknown` 与 `offline`；核心 presence 数据未完成加载前保留缓存顺序，不逐条重排，也不会把尚未返回的数据误显示为离线。
- 增加完整数据批次的一次性排序与本地顺序缓存，并将“最后确认”统一显示为分钟、小时或天/小时/分钟，避免出现数万分钟前的不可读文案。
- 本次只修改搭子列表展示、加载状态和本地 Dashboard 顺序缓存，不改变心跳阈值、Focus、Auth、Todo、Presence 协议或其他同步数据。

## v0.23.280 — 回退到稳定基线并支持自动升级

- 保持 v0.23.277 的应用功能基线，撤回之后的搭子排序及相关后续改动；仅提升发行版本号至 v0.23.280，确保自动更新能够识别并升级到本次稳定回退版。
- Todo 的 Asia/Shanghai 日程修复继续保留；不会修改或重置 Focus、Presence、Heartbeat、Auth、资料、设置或其他同步域。

## v0.23.277 — 完整统一待办日程时区

- 将 Todo 的新建、编辑、提醒提前量、恢复、过期隐藏、紧凑待办筛选和云端上传统一到同一套 Asia/Shanghai 日历规则；不会再出现写入是 `+08`、提醒或显示却按另一台电脑/后台线程的 UTC 时区判断的分裂状态。
- 该修复只触及 Todo 本地存储及其独立同步队列；不会修改或触发 Focus、Presence、Auth、资料、设置或其他同步域。

## v0.23.276 — 修复跨设备待办时区一致性

- 待办日程现在固定采用六毛日历时区（Asia/Shanghai）编码到云端；不再依赖某台电脑、后台工作线程或构建环境的系统时区，避免同一条 12:30 在另一台设备变为 20:30。
- 补充 UTC 同步工作线程的回归测试，并修正仅有提醒、没有事项时间的 Todo 不会误显示在日程时间轴上。

## v0.23.275 — 修复待办时间被云同步偏移八小时

- 修复 Todo 云同步将无时区的本地日程误作为 UTC 写入，导致填写的 12:30 在客户端显示为 20:30、编辑后又被云端覆盖的问题。
- 现在待办的显式日期和时间是所有界面的唯一显示来源；云端 `due_at` 只保存带本地时区偏移的绝对时间，用于提醒和跨设备同步，不再反向改写用户输入的墙上时间。
- 启动时会一次性校正旧版受影响的已排程 Todo，并通过独立 Todo 队列安全回传；此修复不读取、修改或重置 Focus、Presence、Auth、资料、设置或任何其他同步域。

## v0.23.274 — Todo 独立跨设备同步

- 新增独立的 Todo 云端同步域：Todo 只使用 `todos` 表、独立同步队列和独立复合游标，不修改 Auth、Presence、Heartbeat、Focus、FocusSegment、统计、资料、设置或设备数据。
- 新增客户端 UUID、增量 push/pull、逐条 Last Write Wins 合并、幂等 upsert、软删除和失败重试；断网、超时、认证失败或异常数据只会让 Todo 保持待同步，不会注销用户或触发全局数据重置。
- 现有本地 Todo 继续先写入本地，远端合并只更新对应 Todo；旧版非 UUID 待办会一次性迁移为稳定客户端 UUID，避免重复上传。
- Supabase 迁移与部署工作流独立于现有同步工作流，启用 RLS 并按 `auth.uid() = user_id` 隔离数据；Todo RPC 只触碰 `public.todos`。
- 增加网络失败、认证失败、重复提交、冲突合并、软删除、复合游标、坏数据隔离及“不得污染其他同步域”的回归测试。

## v0.23.273 — 详细待办与紧凑待办完整同步

- 紧凑待办不再只显示未来 7 天的普通任务；详细待办中心中所有未完成的普通待办都会显示在六毛旁边，远期任务以“还有 N 天”标注，并可通过原有滚动查看。
- 倒计时和纪念日继续沿用既有的临近提醒或手动置顶规则，避免未临近的日期自动占满紧凑待办。
- 补充远期详细待办到紧凑面板的回归测试；不改变透明桌宠、待办存储、提醒、专注或同步逻辑。

## v0.23.272 — 在线状态超时不再误显示为离线

- 心跳或 Dashboard 缓存超时只表示当前设备暂时无法确认最新状态，不再把已经见过的搭子直接改成“已离线”；有历史登录证据的搭子显示为“正在休息/同步中”。
- 兼容缺少 `last_seen_at` 的新旧 Dashboard 响应，保留本次会话中已确认的 presence 证据；只有明确从未上线、没有任何证据的账号才显示“已离线”。
- 增加后台 refresh token 恢复线程；认证状态短暂失效时自动恢复并立即续上 heartbeat、Dashboard 和个人同步，不阻塞桌面 UI，也不记录或上传密码和令牌。
- 补充 presence 回归测试，覆盖缓存过期、字段缺失、nullable `presence_never_seen` 和认证恢复场景。

## v0.23.271 — 普通窗口不再连带隐藏六毛

- 普通最大化、最小化、隐藏中的工作报告、Codex、浏览器和远程控制窗口不再触发六毛整体隐藏。
- 六毛继续保持非激活置顶，不调用 `raise_()` 或 `activateWindow()`，不会抢占用户焦点。
- 只有真正的 PPT 放映、视频或游戏全屏才会自动隐藏；全屏退出后仍按原状态恢复。
- 增加最小化/隐藏窗口句柄的回归测试，避免 Windows 窗口切换期间误判为全屏。

## v0.23.270 — 暂停后立即冻结跨设备今日工作时长

- 暂停或结束工作时立即发送 inactive heartbeat，不再因为本地 FocusSegment 等待上传 ACK 而继续保留工作中的实时心跳。
- 将在线状态和事实上传彻底解耦：FocusSegment 仍会独立重试、幂等确认，但不会再让两台电脑都暂停后“今日已工作”继续增长。
- 暂停、结束和正常退出都立即触发后台同步，减少跨设备状态收敛等待；不改变 canonical FocusSegment、历史统计或排行榜口径。

## v0.23.269 — 继续降低自习室、报告与快捷入口卡顿

- 优化专注分析摘要：全年连续性统计改为一次构建日度区间并集，避免随着历史记录增加反复扫描同一批 FocusSegment。
- 报告后台只读副本改为按报表所需字段隔离复制，不再复制无关的上传确认指纹，减少打开报告时的主线程开销。
- 自习室首次打开先完成首屏显示，再延迟刷新分析指标；周统计标签不再跟随 250ms 桌面计时器重复聚合，改为低频刷新，不影响实时计时和同步。
- 自习室剩余页面、工作报告剩余页签分帧构建/渲染，避免连续零延迟任务占满事件循环；不改变 FocusSession、FocusSegment、Supabase、闹钟或快捷按钮功能。

## v0.23.268 — 降低界面卡顿并异步生成报告

- 打开搭子自习室时先显示轻量窗口，再分轮创建首页、互动、专注和我的页面，避免一次性创建所有滚动列表、表单和卡片阻塞首帧。
- 工作报告改为后台线程生成只读快照，当前页优先、其余页面分轮渲染；重复刷新会合并，报告窗口打开和日期切换不再长时间占用主线程。
- Dashboard 应用合并同一事件循环中的重复结果，并在批量更新列表时抑制中间绘制；快捷按钮和首页刷新不再因重复布局产生明显抖动。
- 今日跨设备时长投影增加短周期本地缓存，只在事实或状态变化后重新计算区间并集，不改变每秒本地计时、不增加 Supabase 轮询，也不改变 canonical FocusSegment 数据。
- 后台报告线程通过 Qt GUI 线程槽安全回传结果，关闭窗口或快速重复打开时丢弃过期报告，保持现有时间统计、账号同步、透明桌宠和其他业务行为不变。
...
- 增加确定性的 FocusSegment ID，重启、重试和历史恢复可以幂等去重；服务端 ACK 返回的 `accepted_segment_ids` 才会推进本地同步确认。
- 新增升级后的“自动恢复历史专注记录”设置和“立即检查并恢复”按钮。只从本地能确认 `start_at/end_at` 的完整区间恢复；只有累计秒数的旧标量只记录跳过原因，不伪造 FocusSegment。
- 首次启动扫描 account-scoped work session、当前设备 lifecycle sealed 记录和 pending 状态；重复扫描不会重复计时，网络失败保留 pending 等下次重试。
- 15 分钟 checkpoint 使用统一内部常量；跨午夜和历史回补继续交给同一套 FocusSegment 投影按北京时间切片。
- 保持本地 dashboard 缓存 90 秒，历史恢复扫描只读本地，FocusSegment 上传批次限制为 100，避免增加每秒请求和无必要的 Egress。
- 本版本附带 macOS Apple Silicon 未签名 DMG；安装后首次启动会按设置执行本地历史恢复。无法从现有本地数据确认具体起止时间的历史不会被自动补回。
...
## v0.23.286 — 待办同步与本机显示偏好分离

- 待办内容继续按现有 Todo 同步域跨设备同步；“隐藏待办/显示待办”改为每台电脑自己的本地设置，不上传、不合并、不影响另一台电脑。
- 隐藏后，启动、Todo 同步、聊天新增/修改待办、刷新待办面板均不会再自动把紧凑待办条重新显示；可随时通过“显示待办”手动恢复。
- 不改动 Focus、Presence、Heartbeat、Auth、资料或其他同步数据。

## v0.23.287 — 修复休息状态下心跳中断后长期变黄/离线

- Presence 心跳继续独立于工作状态：正在休息、隐藏桌宠、最小化自习室或托盘运行时，都会持续发送 `working=false / session_active=false` 的在线心跳。
- 新增心跳 ACK 健康检查：超过 60 秒没有一次成功上报时，客户端会重建当前 presence 并在保留本地 refresh token 的前提下自动恢复 Auth；不修改服务器绿色/黄色 TTL。
- 修复“本地会话暂时缺失却被后台当作心跳成功”的误判。它现在会作为可重试认证故障进入自愈，而不会让页面静默停留在“正在休息”直到心跳过期。
- 本次不修改 Focus、FocusSegment、Todo、资料、好友、排序或任何 Supabase 生产数据。

## v0.23.288 — 自动暂停跨设备封存与心跳状态一致性

- 自动暂停、手动暂停和完成工作现在会在同一个本地状态切换中，立即用 `working=false / session_active=false` 覆盖后台保留的 active heartbeat；即使 Auth 正在恢复或自习室同步线程尚未结束，也不会继续把已暂停的设备上报为工作中。
- 自动暂停前按空闲、熄屏或休眠截止时刻封存的 FocusSegment，会被标记为优先同步事实，并通过既有串行 delta + 严格 ACK 流程上传；不会与既有 Focus cursor 竞争，也不会丢弃暂停前已累计的时间。
- 若暂停恰好发生在上一轮自习室同步中，封存记录会保留到该轮结束后立即进入下一次串行上传；另一台电脑在下一次读取服务器投影时会以封存区间替换旧 live interval，最终累计时间收敛。
- 保持现有 online TTL、Supabase 表结构、RLS、Auth、Todo 和其他同步域不变；本次只收紧 Presence 与 FocusSegment 的生命周期交接。
## v0.23.289 — 修复 Mac 心跳停滞与工作报告历史缺口

- 当心跳 worker 仍存活但连续 60 秒没有服务器 ACK 时，自动替换可能卡在网络调用中的旧 worker，并立即以当前真实的工作/休息状态重发心跳；休息、隐藏、最小化和托盘状态仍会持续保活，不调整在线 TTL。
- 新增仅限当前账号、只读的最近 60 天 FocusSegment 分页回补。它按每页最多 500 条拉取仍在服务器上的已封存区间，保留正常 delta cursor，不重置本地账本，也不写入或删除服务器事实。
- 本地专注账本容量由 500 条提升至 2,000 条，避免工作报告在回补后再次裁掉近期历史；14 日之后实际存在于服务器的区间会在首次联网同步后恢复到日历和报告。
