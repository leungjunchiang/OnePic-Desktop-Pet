## v0.23.255 — 明确钱袋商品库存与单价

- 商品名称旁明确显示当前库存，避免库存数量和购买价格混淆。
- 商品卡片单独显示“单价：N 吉他拨片”，购买按钮统一为“购买”。
- 更新荒野小卖部说明文案，并修复钱袋总览布局类型错误导致打开即崩的问题。

## v0.23.254 — 多设备暂停后今日时长最终一致

- 暂停和完成专注后立即触发 FocusSegment 增量上传与账号状态刷新，不再等待下一轮 30 秒同步。
- 升级后自动执行一次有界 FocusSegment reconciliation，修复旧 cursor/本地缓存漏段；服务器有效总量领先本地封存并集时也会自动排队修复。
- 两台电脑暂停后统一使用服务器账号级有效今日时长校准显示；该值只用于 UI 收敛，不会伪造本地 Segment。
- 增加 60 秒强制审计冷却和回归测试，避免反复暂停造成全量历史请求。

## v0.23.253 — 统一暂停气泡与底部今日工作时长

- 暂停、完成、菜单、日报和成长提示统一使用与桌面计时条相同的可见账号今日时长。
- 修复本地兼容累计高于跨设备区间并存时，暂停气泡显示 7小时23分钟、底部计时却为 7:21:51 的问题。
- 在暂停/完成边界记录本地日汇总、跨设备区间、服务器有效时长、已封存段和实时投影数量，便于继续审计时间差异。

## v0.23.252 — 同步自习室与桌面六毛的实时今日时长

- 自习室首页每秒接收与桌面六毛相同的本地实时账号时长投影。
- 修复网络同步间隔期间首页继续显示旧缓存，导致比桌面六毛少几十秒或少一分钟的问题。
- 不增加服务器请求；跨设备数据仍按原有后台同步节奏更新。

## v0.23.251 — 修复今日工作计时停顿与倒退

- 专注中的“今日已工作”改为本地单调时钟连续推进，服务器/缓存暂时不更新时也不会停表。
- 服务器时间只允许向上校准；旧快照不会让正在工作的时间倒退或重新卡住。
- Qt 主线程短暂延迟后会一次性显示真实经过的秒数，不再慢速追赶；暂停、结束、切账号和跨日仍会重新锚定权威值。
- 增加网络投影冻结、GUI 延迟和旧服务器值回退的回归测试。

## v0.23.250 — 修复自定义音乐试听卡死与卡顿

- Windows 自定义音乐改由独立 Qt 音频助手进程播放，主界面不再直接进入可能阻塞的原生解码/停止调用。
- 试听或闹钟关闭时只结束音频助手，不等待底层播放器，避免六毛窗口未响应；MP3、WAV、M4A、AAC 等长音频不再以 MCI 作为主播放后端。
- 增加音频助手生命周期与播放后端选择回归测试。

## v0.23.249 — 修复自定义铃声试听闪退

- 保留 Windows Qt 音频工作线程后端，直到其原生 `QThread` 真正发出 `finished`，避免试听停止或自动结束时线程仍在运行却被销毁，导致程序闪退。
- 补充自定义音频试听的异步停止生命周期回归测试。

## v0.23.248 — 启动流畅度与社交同步降载

- 启动后的首个 Dashboard 先完成在线状态、搭子状态和服务器时间显示，完整 FocusSegment 历史同步延后到界面稳定后执行，避免启动阶段与 GUI 抢占资源。
- 没有新的本地专注段、账号切换或认证恢复时，不再每分钟重复执行 FocusSegment 增量同步和完整性审计；服务器有效今日时长与实时心跳仍按原频率刷新。
- 本地开始/暂停/恢复/完成专注、退出保存以及登录恢复仍会立即触发段同步，保证工作时间不会因降载而延迟上传。

## v0.23.247 — 宠物计时与自习室服务器时间对齐

- 服务器确认的账户级今日累计到达后，桌面宠物气泡立即采用同一基线，不再以每秒一格的平滑追赶造成几十秒到几分钟的显示滞后。
- 跨设备实时工作段使用 Supabase 返回的服务器时钟裁剪和计算；两台电脑本地时间有偏差时，不会再把同一账号的当前工作段显示成不同长度。
- 本机仍只在服务器确认基线之上补充尚未确认的当前工作秒数，暂停、恢复和多设备重叠不会重复累计；服务器不可用时保留最近一次已确认值。
- 增加服务器时间偏移和即时基线同步的回归测试，覆盖截图中的 4:10 与 4:11:56 这类跨设备差异。

## v0.23.246 — 社交认证自动恢复与同步看门狗

- 本地仍有 refresh token 时，社交后台不再因一次认证刷新错误永久停止；客户端会在后台线程自动重连，恢复后重新启动心跳、Dashboard 和 FocusSegment 同步，不要求用户反复退出登录。
- 独立 heartbeat 增加服务器 ACK 看门狗；连续 60 秒没有成功回执会主动用最新工作状态唤醒重试，避免“界面显示已登录、服务器却停在昨晚”的双向断联。
- Dashboard 断线缓存不再把所有搭子伪造成“已离线”，改为保留最后确认的在线/工作/今日时间并标记“状态待确认”；恢复连接后自动替换为服务器实时结果。
- “检测自习室网络”新增认证、心跳、首页读取、专注区间四条链路的最后成功时间与错误状态，便于区分上行、下行和认证问题。

## v0.23.245 — Windows 闹钟音频与开工动画流畅度修复

- Windows 自定义闹钟音频优先使用独立 Qt Multimedia 工作线程，MP3/WAV/M4A/AAC 不再依赖 MCI 作为主播放路径；MCI 仅在工作线程后备失败时使用，GUI 不执行同步 `QMediaPlayer.stop()`。
- 闹钟试听与实际响铃共用非阻塞音频后端，保留解码失败时的多级回退和自动停止清理。
- “开始工作”改为固定目标帧的单一交叉淡化：目标帧在过渡期间冻结，过渡完成后才启动 SIT 动画，避免 `computer` 淡化与 SIT 换帧竞争。
- 增加 `pet.refresh_pixmap` 和慢于 25ms 的重绘诊断日志，便于继续定位高 DPI 或原生遮罩造成的掉帧。
