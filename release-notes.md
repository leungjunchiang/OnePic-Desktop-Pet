## v0.23.38 — 随机点歌与 Windows 网易云启动修复

- 新增陈楚生结构化歌曲目录与洗牌袋随机选歌；一轮内不重复，保留最近播放状态。
- Windows 播放网易云 Deep Link 时优先解析当前 `orpheus` 注册的真实 `cloudmusic.exe`，使用安装目录作为工作目录，并传入正确的 `--webcmd` 参数。
- 避开缺少空格的旧 URL Scheme 命令；客户端启动进程立即异常退出时不再误报为“已播放”，自动回退正版网页。
- “随机听一首”按钮改为使用同一份结构化曲库 Deep Link，不再误走旧版网易云 UI 自动化搜索链路；客户端唤起后通过现有 Transport 补发一次幂等 Play。
- “陈楚生随机电台”优先打开网易云客户端歌手入口并补发 Play，只有客户端唤起失败才回退到官方网页歌手页。
...
## v0.23.38e — 随机歌曲后台播放优先

- 修复网易云 Deep Link 没有建立目标媒体 Session 时，通用媒体控制误发送全局播放键，导致正在播放的 QQ 音乐被暂停而网易云没有开始播放的问题。
- 曲库随机单曲和陈楚生随机电台现在只控制匹配的网易云 Session；没有目标 Session 时改走现有网易云 UI 自动化精确播放/歌手随机播放兜底。
- 两条目标播放路径都失败时返回失败，不再把“已打开链接”伪装成“已经播放”；新增回归测试确保不会发送全局媒体键。
- 曲库路径进一步优先使用后台 UI Automation 的 Value/Invoke pattern，不移动鼠标、不调用鼠标脚本；旧的桌面鼠标兼容脚本不再参与曲库随机播放。
- 修复后台播放失败后网页兜底被提前截断的问题；现在会继续打开网易云官方歌曲搜索页，并保持不影响其他播放器。

## v0.23.38g — 随机电台跟随默认音乐软件

- “随机听一首”继续使用本地曲库选歌。
- “随机电台”按设置中的默认音乐软件打开陈楚生歌手入口，不再因一个平台失败而跨平台抢占其它播放器。
- 网易云使用歌手 Deep Link；QQ 音乐、酷狗音乐、Apple Music 等使用对应官方歌手/搜索入口。
- 客户端入口失败时只回退到同一平台的官方网页。
