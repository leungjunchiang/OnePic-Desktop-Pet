## v0.23.279 — 修复待办在 UTC 环境提前八小时判定

- 将紧凑待办、今日/近期待办筛选、超时隐藏与恢复提醒统一改为直接使用六毛的 Asia/Shanghai 日程时钟；修复 GitHub Windows（UTC）构建环境会把无时区测试时钟误判为八小时后、从而提前隐藏待办或压制未过期提醒的问题。
- 此修复仅触及 Todo 的本地日程判定；不会读写、重置或影响 Focus、Presence、Heartbeat、Auth、资料、设置或其他同步域。

## v0.23.278 — 搭子状态分层排序与可读确认时间

- “我的搭子”现在固定按：当前状态（在线专注 → 在线休息 → 黄灯/离线）→ 今日专注时长 → 本周专注时长 → 最后确认时间排序；不再让单一指标意外反超更重要的状态。
- 最后确认时间改为易读的“刚刚确认 / 8分钟前 / 2小时17分钟前 / 1天4小时32分钟前”格式，避免出现难以理解的大分钟数。
