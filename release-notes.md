...
- 修复聊天在连续歌曲/人物话题后把“人生的意义是什么”误判为陈楚生人物经历的问题；无明确指代时不继承旧话题。
- 为 Codex 当前回合增加换题边界，减少无关知识注入、跑题和不必要的长回答。
- 升级本地 Codex 会话状态版本；旧提示协议只清除 thread 指针并自动建立干净会话，不影响 Todo、聊天记录和工作计时。
- 新增相关意图、知识检索、提示边界与旧会话隔离回归测试。
