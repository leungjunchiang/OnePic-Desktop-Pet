...
- Aura 升级为“底部炫光环 + 周身雾化气场 + 少量亮点粒子”的三层视觉结构，增强存在感与流动感。
- 红、金、蓝、紫四种 Aura 分别加入余烬、星芒、冷光和蓝紫微光风格；保持面部清晰，不改变窗口 geometry、mask 或点击范围。
- 底部光环与雾团继续使用有界 Aura-only 缓存和现有 pixmap compositing；没有新增网络请求、QThread、独立窗口或计时逻辑。

## v0.23.223 — 修复 Aura 不可见与持续动画卡顿

- 修复 Aura 已合成到 pixmap、却被角色轮廓 native mask 裁掉而肉眼不可见的问题；在保留原窗口 geometry 和点击轮廓的前提下，增加避开面部的身体外缘/下方柔光，四种颜色现在均可直接辨认。
- 调整合成顺序：先确定角色、换装或完整动作素材，再绘制 Aura，最后绘制表情符号，避免完整动作素材把 Aura 整层覆盖。
- 持续 Aura 动画由 90ms 降至 180ms；mask key 不再随 Aura phase 或仅颜色变化而失效，同一轮廓不会反复调用原生 `setMask()`。
- Aura 缓存改用实际逻辑尺寸，避免每帧缩放缓存层；仍保持 96 项有界缓存、完全本地和渲染失败回退，未增加窗口、线程、定时器或网络请求。

## v0.23.222 — 本地 Aura / 氛围特效系统

- 新增红色雾气、金色光雾、蓝色柔雾和紫色气场；使用 `QPainter`、径向渐变和确定性微粒在现有角色 pixmap 内合成，不依赖外部素材、独立窗口或网络服务。
- Aura 复用现有动画 tick，支持淡入淡出、高 DPI、缩放、角色翻转及与爱心、问号、怒气等表情特效同时显示；窗口 mask 继续只依据角色图层，雾气不会扩大点击区域。
- 新增“显示与窗口 → 氛围特效”统一菜单，可选择自动、关闭或手动颜色；自动模式根据现有 `PetState` 映射红/金/蓝/紫效果，设置只保存在本机。
- Aura-only 图层使用 96 项有界缓存与尺寸、DPR、强度、透明度、phase 分桶；渲染异常自动回退原始角色帧。未新增 QTimer、QThread、原生窗口、Supabase 请求或其他网络行为。
...
- Supabase 严格 delta RPC 在空增量时保留原 cursor，避免并发封存的 FocusSegment 被请求水位永久跳过；不改写 `end_at`，不上传进行中的 FocusSession。
- 增加回归测试，覆盖空 delta、跨设备显示保持和低 Egress 行为。
...
- 工作报告、六毛和自习室继续使用同一份账号级区间并集；报告不再用本机日/周兼容汇总覆盖远端设备实时投影，因此今天、昨天及任意报告区间都能保持同一口径。
- Supabase 的今日/本周统计与搭子排行榜改为读取现有 `lili_focus_device_presence` 的全部新鲜设备，并与封闭片段做并集；两台电脑重叠工作只计算一次，原始 FocusSession 不被高频改写。
- 新增同步诊断日志：`cursor_before → upload_count → returned_count → merge_count → cursor_after`，便于区分上传、服务端返回和本地合并哪一环遗漏。

## v0.23.203 — 稳定 macOS Intel 发布构建

- 修正 macOS Intel 低空间/非零虚拟屏幕坐标下的待办面板回退定位，优先保持与六毛可见区域的安全间距，避免边缘几何误判阻断安装包构建。
- 工作报告的跨设备实时投影、可移动时间窗和账号级 FocusSession 区间并集功能保持不变。

## v0.23.202 — 跨设备实时专注与可移动工作报告

- 新增每设备临时实时专注投影：同一账号在两台电脑同时工作时，六毛和自习室可看到两台设备当前正在增长的区间；重叠部分按账号级区间并集只计算一次。
- 投影只服务于实时显示，暂停/结束后的正式 FocusSession 仍按原流程封存，未把 FocusSession 改成高频写入。
- 工作报告保留日 / 周 / 月 / 年自然周期默认入口，并新增左右箭头、图表拖动按天平移、点击日期标题选择精确起止日期；任意区间统一使用同一套 FocusSession interval union。
- 月/年窗口离开自然周期后分别按 30 天 / 365 天滚动，随时可以一键回到本月或今年；不新增永久“自定义”页签。
- 延续 v0.23.201 的专注片段增量同步，避免为了实时显示重新传输整段历史。

## v0.23.201 — 专注片段增量同步，降低多设备网络流量

- 新增专注片段增量同步协议：首次登录/新设备获取一次近 400 天快照，之后按服务端游标只接收新增或实际变化的片段；两台电脑的账号级区间并集与显示口径保持不变。
- 旧版 `lili_sync_focus_segments` 保留兼容返回格式，但重复上传相同事实不再反复刷新 `updated_at`，避免旧客户端制造无意义的增量变更。
- 增量游标只保存在账号级本地同步元数据中，沿用现有 `merge_remote_segments` 原始事实合并路径；不修改 FocusSession、计时结束规则、Presence 或统计口径。
- 保留旧中转服务回退路径；当服务端尚未部署新 RPC 时自动使用旧接口，升级后再切换为增量同步。

## v0.23.200 — 修复未来结束片段导致的跨设备今日显示不一致

- 修复计时检查点与暂停/闲置切换发生竞态时，某条专注片段的结束时间暂时落在当前时刻之后，导致跨设备今日专注并集整份回退为旧缓存的问题。
- 显示层现在只对这类已关闭异常片段创建内存副本，并将结束点截到当前时刻后参与并集；原始 FocusSession、同步事实、统计聚合和上传内容均不修改。
- 保留未来开始、负时长和超长片段的严格数据质量保护，并增加回归测试，避免显示容错演变成错误计时。

## v0.23.196 — 修复跨设备今日专注显示回落

- 修复自习室首页“我的今日专注”和六毛实时“今日已工作”在收到一次账号级合计后，因后续本地状态刷新又回落为单设备时长的问题。
- 保留最后一次通过校验的跨设备 FocusSession 区间快照；没有新区间响应时只做只读重投影，并叠加本机新完成区间，重叠区间仍只计算一次。
- 网络、查询或数据校验失败时不清空有效显示快照；首次失败仍回退现有本机显示并记录本地诊断日志。工作报告、历史/周统计、排行榜、缓存、FocusSession 写入同步和 Supabase RPC 均未改动。
...
- 自习室关闭后改为安全隐藏并复用；停止窗口级刷新定时器，等待后台线程通过正常 `finished` 回调清理，重新打开时自动刷新。
- 增加关闭后窗口仍可复用的回归测试；不改变专注时间、Presence 或云端数据同步逻辑。

## v0.23.185 — 年度工作报告

- 工作报告新增“年度”页签，形成日度｜本周｜月度｜年度四级时间视图。
- 年度页提供全年工作日历热力图、12 个月累计工作趋势、全年典型工作节律和年度里程碑。
- 工作时长口径统一为“工作日日均”，避免与自然日日均混淆；年度统计仍只读取当前账号的原始专注区间，不新增云端统计表或请求。
- 优化区间聚合：直接将已合并的专注区间切分到小时/日期桶，年度报告不会因逐小时扫描全年记录而拖慢刷新。

## v0.23.184 — “XX家的六毛”名称语义修复

- 将可编辑字段统一为“六毛主人名”，只控制“XX”，固定显示后缀“家的六毛”。
- 搭子名称显示优先级统一为当前用户私人备注 → 对方 `owner_nickname` → 对方账号昵称；旧 `pet_name` 只保留协议兼容，不再作为可编辑宠物名或显示来源。
- 保存设置时分离账号昵称与主人名，支持清空主人名并回退到账号昵称；不修改 FocusSession、统计算法或数据库结构。

## v0.23.183 — 同账号多设备 Presence 与专注区间并集

- 同一账号支持多台设备独立上报 Presence；账号在线/工作状态按所有新鲜设备聚合，本机开始/结束按钮仍只控制本机 FocusSession。
- 保留每台设备的真实 FocusSession 原始事实，重叠专注只在统计层按区间并集计算；重复同步继续幂等，历史事实不回填、不重写。
- 修复 Dashboard 短暂失败期间把最近可信搭子状态显示成离线、把未知专注人数显示成 0，以及健康响应后恢复提示不消失的问题。
- Dashboard 30 秒、Reaction 60 秒、排行榜 600 秒和 Heartbeat 约 15 秒节奏保持不变；增加脱敏 Dashboard 错误分类日志。
- 不包含第三阶段“XX家的六毛”名称 UI 修改。

## v0.23.180 — 搭子双向在线、名称格式与北京时间修复

- 搭子名称统一显示为“名称家的六毛”：优先使用当前账号给对方的私人备注，其次对方自定义六毛名，再次对方公开名，全部未设置时显示“搭子家的六毛”。
- 修复客户端重启后心跳序号从 1 重新开始、服务端返回 200 却拒绝旧序号的问题；本地持久化服务端序号，数据库按同设备或超时换机安全续接。
- 修复空闲账号的 `session_id = NULL` 令整个自习室 dashboard 变成 NULL 的问题，在线、休息、专注状态恢复实时刷新。
- 为仍在运行的旧版客户端增加仅限本人行的 Presence 兼容保护，由 RLS、服务端时间戳、单调序号和房间成员校验约束；持续的 403/42501 已恢复为 200。
- 工作报告统一按北京时间解析旧版无时区时间；修正 QQ 账号 00:14–01:30 被误显示成 08:14–09:30 的记录，并防止旧客户端重新覆盖校正值。

## v0.23.179 — 搭子在线状态与昵称快照修复
- 修复账户 dashboard 与自习室 room dashboard 合并时，后一个投影省略 `pet_name`/昵称字段导致搭子名称刷新后消失的问题；只有服务端明确返回 `null` 才会清除名称。
- 保持显示优先级：我给搭子的私人备注 → 对方给自己六毛起的名字 → 对方公开昵称家的六毛。
- 心跳统一使用 `lili_upsert_focus_presence` 原子 RPC，不再尝试直接写入受保护的 `lili_focus_presence` 表；继续由服务端时间戳和两分钟窗口判断在线/专注/工作状态。
- 停止轮询当前项目不存在的可选成果见证 inbox RPC，避免每次刷新产生无意义的 404 错误。

## v0.23.178 — 搭子昵称优先级与自习室实时状态

- 搭子卡片、打字自习室、排行榜和房间动态统一使用“我给对方的私人昵称 → 对方给自己的六毛昵称 → 搭子家的六毛”的显示优先级。
- “我的账号”增加可选的六毛昵称；清空后自动回退到“搭子家的六毛”，并兼容旧版中转服务。
- 登录后的 Presence 心跳继续作为在线状态来源；房间卡片明确显示“正在工作/正在休息/已离线”，并保留服务端两分钟新鲜度窗口。
- 新增 Supabase 迁移，为已授权的 dashboard 与 room dashboard 返回对方自定义六毛昵称；不改变私人备注的访问范围。

## v0.23.175 — 自习室性能与界面显示优化

- 根据运行日志修复报告窗口每 5 秒重新计算并重建三页控件造成的 GUI 卡顿；报告自动刷新改为低频更新，用户主动打开或结束工作时仍立即刷新。
- 延后首次自习室缓存快照的重布局，并将窗口激活放到显示后的事件循环，减少点击快捷按钮进入自习室时的阻塞。
- 修复月度工作日历的日期槽位偏移和 `QRect` 边界计算，完整显示最后一列和最后一行，并增加边界回归测试。
- 搭子名称在没有私有备注时补充读取公开 `display_name`；私有备注仍只对当前账号生效并优先显示。
- 移除首页来源不明确的“可见搭子合计”，保留明确的在线搭子数量和本人今日专注时间；房间详情中的共同专注统计保持不变。

## v0.23.174 — 统一社交身份与专注时间

- 统一搭子卡片和排行榜的身份显示：当前账号有“仅当前账号可见的私人备注”时优先显示备注，没有备注时回退到对方公开昵称；排行榜中的本人使用本人名称并标记“（我）”，不再退化为“搭子家的六毛”。
- 工作报告、搭子自习室和六毛宠物下方的今日/本周专注时长统一消费共享的 FocusSession 时间投影，避免同一会话在不同页面显示不同结果。
- 增加社交身份与工作报告统计的回归测试，覆盖私人备注优先、本人排行榜名称以及日/周投影一致性。

## v0.23.168 — 禁止旧缓存伪造本周专注时长

- 修复排行榜显示整整 168 小时的问题：没有有效 FocusSession 时，旧的 `lili_focus_daily`、profile 周缓存和客户端计时缓存一律不再参与统计，结果为 0 分钟。
- 保留有效片段的跨北京时间切片、重叠去重、异常区间过滤和统一排行榜/搭子/工作报告口径。
- 在线清理没有原始片段但残留 604800 秒缓存的账号，清零当前周派生数据。
- 增加“完整周缓存不能生成 168 小时”的本地回归测试。

## v0.23.167 — 统一专注统计口径并修复跨日/污染数据

- 今日、本周、搭子卡片、排行榜和工作报告统一以有效专注片段为唯一统计源；旧的 profile/daily 汇总不再作为统计来源。
- 专注片段按北京时间自然日切片，跨午夜的工作只计入实际相交的两天；当天没有有效片段时明确显示 0 分钟，不再复活旧缓存。
- 排除未来、负时长和超过 24 小时的异常片段，并保留数据质量原因，避免重复结束、崩溃恢复或跨日累计值污染统计。
- 同步专注片段后自动回算本地/云端兼容汇总，修复已有的今日/本周错误时长；补充毛毛冲等账号的历史数据回算。
- 增加跨日、过长片段、空白当天清零以及统一排行榜/搭子口径的回归测试。

## v0.23.166 — 修复搭子在线状态与今日时长

- 修复跨北京时间日期的旧 presence 心跳把 0 分钟显示成数小时的问题。
- 修复设备会话保护导致心跳 HTTP 400、双方明明在线却互相显示离线的问题。
- 为每个账号保存稳定设备 ID，首次成功后续租不重复抢占其他设备。
- 更新 Supabase 直连及三类中转心跳字段，并补充在线状态回归测试。

## v0.23.165 — 修复搭子账号数据串用与在线状态

- 延续 v0.23.164 的社交修复：未登录过的搭子仍显示卡片时，今日/本周/本轮专注时长统一为 0 分钟。
- 未设置私人备注时恢复公开昵称，因此“小梁”显示为“小梁家的六毛”；账号切换不会沿用旧账号资料。
- 登录后正常发送 presence 心跳，双方在线状态可以恢复；缓存按 Supabase 用户 ID 和房间隔离。
- 包含 Supabase 兼容迁移、社交回归测试，以及跨平台窗口几何测试修正，确保 Windows/macOS Release 可以完整构建。

## v0.23.164 — 修复搭子账号数据串用与在线状态

- 桌面自习室缓存改为按 Supabase 用户 ID 和房间隔离；旧的无账号缓存不再被新账号读取，避免把其他账号的专注时长、娃衣或备注带过来。
- 从未发送过 presence 心跳的搭子，今日/本周/本轮专注时长统一显示为 0 分钟；数据库和客户端同时做兼容处理，避免沿用 profile 中的旧累计值。
- 未设置私人备注时清除缓存中的旧备注，恢复使用公开昵称，因此“小梁”显示为“小梁家的六毛”。
- 修复认证管理器已有会话但传输层 session 尚未同步时跳过心跳的问题；登录后会正常刷新 `last_seen`，双方在线状态可以恢复。
- 增加账号切换、未登录搭子、备注清除和心跳会话同步回归测试，并为 Supabase 部署增加对应迁移工作流。

## v0.23.163 — 闹钟关闭时彻底停止音乐

- 关闭、开始工作或稍后提醒时，闹钟弹窗会立即隐藏，并异步停止当前及残留的 Windows MCI 音频别名，不阻塞 Qt 主线程。
- MCI 停止/关闭命令增加 `wait` 与兼容重试及返回码日志，避免弹窗已关闭但自定义铃声仍继续播放。
- macOS/Linux 的自定义闹钟不再只静音而继续循环，关闭后会真正停止 `QMediaPlayer`。
- 增加闹钟音频停止竞态回归测试。

## v0.23.162 — 工作报告与桌面待办并行显示

- 打开工作报告时不再主动隐藏六毛旁边的桌面待办浮层，两个窗口可以同时使用。
- 移除工作报告打开期间阻止“显示待办”的旧保护，待办被手动隐藏后仍可正常恢复。
- 修复工作报告关闭恢复待办时的 Qt 事件时序问题，并增加工作报告/待办并行显示回归测试。

## v0.23.161 — 修复登录后重要日期待办不显示

- 修复应用先以匿名数据启动、登录后切换到账号数据时，桌面待办面板仍停留在“空且隐藏”状态的问题。
- 桌面待办、启动恢复、账号切换和“显示待办”统一使用同一份桌面投影；待办中心中的未完成重要日期可以正确恢复到六毛旁边。
- 让倒计时/纪念日的 `show_on_desktop` 显式设置真正参与桌面待办筛选。
- 增加重要日期独立显示、登录后空面板恢复和显式桌面事件的回归测试，并保留 `todo.show.request/result` 生命周期诊断记录。

## v0.23.160 — 修复快捷按钮悬停闪烁与布局跳动

- “开始工作/继续工作”悬停时直接进入工作报告二级状态，只显示“工作报告”，不再先闪现整排二级按钮。
- 一级快捷按钮与工作报告使用独立容器；切换悬停状态时不再重建、移除或复用一级按钮布局，避免按钮短暂跑到二级位置。
- 工作按钮与工作报告组成连续悬停区域，保留短暂的指针过渡时间；离开整个区域后才收起工作报告。
- 增加快捷面板回归测试，覆盖二级状态、按钮父容器、悬停防抖和一级按钮位置稳定性。

## v0.23.159 — 修复自定义闹钟点击卡死与过期重放

- Windows 自定义闹钟铃声改用独立的 winmm/MCI 通道，播放与停止不再进入 Qt Multimedia 的同步 `QMediaPlayer.stop()` 路径；弹窗按钮可以立即返回 Qt 主事件循环。
- 关闭、开始工作、稍后提醒统一走幂等的异步清理链，保留生命周期日志，避免重复弹窗和重复音频清理。
- 软件重启后不再把软件未运行期间错过的旧闹钟补响到下午；软件持续运行时仍保留“迟到 10 分钟后 30 分钟重试”的规则。
- 增加 Windows 自定义音频回归测试，并验证完整测试套件。

## v0.23.155 — 稳定跨平台待办显示构建

- 保留 v0.23.154 的待办显示规则：已读但未完成的事项持续显示，完成或删除后才消失。
- 放宽 macOS Intel 原生窗口坐标的单像素舍入断言，避免与待办无关的 DPI 差异阻断 Release 构建。

## v0.23.154 — 校正已读待办的回归测试

- 更新紧凑待办回归测试，使其与新的产品规则一致：已读但未完成的待办仍保持显示，完成或删除后才消失。
- v0.23.153 的实现逻辑不变，仅补齐测试期望，确保 Windows/macOS Release 构建可以完整通过。

## v0.23.153 — 待办自动显示只以“完成/删除”为结束条件

- 桌面待办条现在显示所有未完成事项；在详细待办中标记“已读”不会再把仍未完成的事项从六毛旁边收起。
- 点击待办圆圈、在详细待办中选择“已完成”，或删除事项后，待办才会从桌面条消失。
- 重启、定时刷新和手动“显示待办”统一使用同一条未完成待办投影，避免已读状态导致面板看似失效。
- 增加回归测试，覆盖已读但未完成事项保持显示，以及完成后正确消失的路径。

## v0.23.152 — 修复“显示待办”无法恢复面板

- 右键选择“显示待办”时，即使未完成事项此前已标记为已读，也会重新显示在六毛旁边。
- 自动启动和定时刷新仍只展示未完成、未读事项；全部已读时继续自动隐藏，保持桌面整洁。
- 手动恢复后的待办仍支持勾选完成、编辑和删除，隐藏待办后恢复为未读筛选。
- 增加 TimeMemory 与桌面窗口回归测试，覆盖已读未完成事项的手动恢复路径。

## v0.23.151 — 加固 Windows/macOS 生命周期与 Codex 回调

- Qt 原生事件、启动检查和 Codex 后台线程回调增加统一故障边界；单个过期窗口/线程信号失败时不会把桌宠进程带走。
- Qt 事件异常现在作为已处理事件结束，避免 Windows 调试器或关闭中的 QObject 让事件分发器提前退出。
- 应用数据目录新增大小受限的 `runtime.log`，记录未处理异常和生命周期异常，便于定位无控制台打包版本的退出原因；日志不写入仓库或私有素材。
- 检测到事件循环在未收到用户退出请求时意外返回，会恢复一次桌宠窗口；明确退出、系统关机和更新重启路径不变。

兼容发布检查：启动更新检查继续保持“仅提示、不自动安装”的既有行为，并通过生命周期保护包装回调。

## v0.23.96 — 修复待办面板显示与手动恢复

- 空待办时只隐藏紧凑待办面板及其操作按钮，不再把空状态误当成永久隐藏。
- 修复已有未完成待办时点击“显示待办”无法恢复面板的问题；手动显示会重新读取当前待办并恢复面板。
- 新增待办后会按当前自动显示设置恢复面板；用户明确选择“不自动显示”时仍尊重该设置。
- 增加回归测试，覆盖已有待办被手动隐藏后再次显示的场景。

## v0.23.95 — 修复实时报告累计时长污染

- 修复工作报告把暂停/检查点后的累计 Session 时长误画成当前连续工作区间，避免日报和周报凭空出现 3 小时等错误数据。
- 日报、周报和心跳现在以北京时间下的本地 FocusSession 原始记录为基准，仅叠加当前正在运行的实际计时片段。
- 修复恢复/检查点后的会话开始时间，避免旧累计值被当作在线状态的起始时间。
- 工作时长柱状图纵轴按最大值向上取整并留出一小时空间，柱顶不再贴住上限；增加数据一致性回归测试。

## v0.23.94 — 自习室导航与报告图表自适应修复

- 四个自习室主选项卡始终等宽，随窗口大小重新计算，不再出现切换页面后某个选项卡突然变窄。
- 周/月工作时长图表纵轴按当前最大值向上取整后额外保留 1 小时空间，柱顶不再贴住图表边界。
- 缩小周/月图表横轴下方的无效留白，同时保留日期、星期和小时刻度的可读区域。
- 更新检查增加安全的 HTTPS EOF/连接重试、GitHub 请求头和短连接设置，降低 Windows 网络环境下偶发的 SSL 读取失败。

## v0.23.93 — 报告坐标轴与自习室布局优化

- 月度小时图保留 24 根小时柱，仅显示 00:00、03:00……24:00 刻度；悬停显示完整小时区间和本月累计专注。
- 周度工作时长纵轴自动多留一档空间，避免最高柱顶到边界；工作节奏图移除整行背景带，只保留实际时间区间块。
- 自习室首页、互动、专注、我的四个主选项卡按窗口宽度均分，宽窄窗口都能完整显示文字；搭子卡片新增“请蛋糕”。
- 缩短私有素材备份时间戳，避免 Windows 深层临时目录触发路径长度限制。
...
## v0.23.169 — 统一专注统计边界并降低 Qt 主线程负担

- 修复 Supabase 在 UTC 数据库中把北京时间周一 00:00 错换成 08:00，导致排行榜漏掉周一前 8 小时的问题。
- 统一 FocusSession 日/周回算与排行榜的 Asia/Shanghai 半开区间，继续以原始区间并集为唯一统计源。
- 报告不再使用不一致的 FocusSession/profile 快照覆盖原始区间统计。
- 每秒计时路径缓存已关闭区间投影，不再每秒扫描完整专注记录。
- 增加低开销 Qt event-loop lag、计时器清单和关键回调耗时诊断；报告快照未变化时不再重建整棵页面控件树。
## v0.23.173 — 修复社交身份回退并降低桌宠菜单卡顿
