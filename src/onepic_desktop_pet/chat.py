...
class AISettingsDialog(QDialog):
    ...
    focus_history_recovery_requested = Signal()
    ...
    def __init__(
        ...
        self.auto_recover_focus_history = QCheckBox("自动恢复历史专注记录")
        self.auto_recover_focus_history.setChecked(
            getattr(settings, "auto_recover_focus_history", True)
        )
        self.auto_recover_focus_history.setToolTip(
            "应用更新或异常恢复后，自动检查本地尚未同步的专注记录，"
            "并在能够确认起止时间的情况下恢复并上传。无法验证的累计时间不会自动写入。"
        )
        form.addRow("", self.auto_recover_focus_history)
        self.recover_focus_history_button = QPushButton("立即检查并恢复")
        self.recover_focus_history_button.setObjectName("softButton")
        self.recover_focus_history_button.setToolTip(
            "重新扫描本地可验证的起止区间；反复点击不会重复计时。"
        )
        self.recover_focus_history_button.clicked.connect(
            self.focus_history_recovery_requested.emit
        )
        form.addRow("历史恢复", self.recover_focus_history_button)
        idle_hint = QLabel(
    ...
    def apply(self) -> None:
        ...
        self.settings.auto_recover_focus_history = self.auto_recover_focus_history.isChecked()
