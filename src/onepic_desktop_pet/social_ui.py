...
def _owner_nickname(record: dict[str, Any] | None) -> str:
    ...
    if not isinstance(record, dict):
        return "æ­å­"
    return str(record.get("owner_nickname") or record.get("nickname") or "æ­å­").strip() or "æ­å­"
    ...
    def run(self) -> None:
        try:
            ...
            if not callable(checker):
                raise SocialError("å½“å‰è‡ªä¹ å®¤åŽç«¯æœªæä¾›å¥åº·æ£€æŸ¥ã€‚", kind="config")
            self.completed.emit(dict(checker() or {}))
        except SocialError as exc:
            self.failed.emit(exc)
        except Exception as exc:
            self.failed.emit(SocialError(f"å¥åº·æ£€æŸ¥å¤±è´¥ï¼š{exc}", kind="network"))
...
class BuddyCardWidget(QWidget):
    ...
    def __init__(self, buddy: dict[str, Any], parent=None) -> None:
        ...
        headline = QLabel(
            f"{'ðŸŸ¢' if online else 'âšª'}  {social_pet_label(nickname)}"
        ...
        if buddy.get("stale_presence"):
            time_text = "ç¦»çº¿ç¼“å­˜ï¼›ä¸Šæ¬¡çŠ¶æ€ä¸è®¡å…¥å½“å‰ä¸“æ³¨"
        else:
            time_text = "ä»Šæ—¥ä¸“æ³¨æ—¶é•¿å·²éšè—" if duration is None else f"ä»Šæ—¥å·²ä¸“æ³¨ {format_work_duration(duration)}"
        ...
        if quick_status and (not expires or expires > datetime.now().astimezone().isoformat()):
            quick = QLabel(f"çŠ¶æ€ï¼š{quick_status[:40]}")
