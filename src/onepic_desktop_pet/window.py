...
- 将“随机听一首”和“随机电台（歌手主页）”分成两条明确的音乐入口；
...
class PetWindow(QWidget):
    ...
    def open_music_collection(self) -> str:
        """打开默认音乐软件的歌手主页，后续随机播放交给音乐客户端。"""

        if self.music_provider_manager.open_catalog_artist_collection():
            self.show_speech("已打开陈楚生歌手主页，后面交给播放器随机播放♪", 3600)
        else:
            self.show_speech("暂时没能打开陈楚生歌手主页，请确认音乐软件或浏览器可用。", 3600)
        return "陈楚生随机电台（歌手主页）"
