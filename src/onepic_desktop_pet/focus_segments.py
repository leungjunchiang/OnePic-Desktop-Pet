...
FOCUS_SEGMENT_CHECKPOINT_SECONDS = 15 * 60


def deterministic_focus_segment_id(
    device_id: str,
    session_id: str,
    start_at: datetime,
    end_at: datetime,
) -> str:
    """Return a retry-stable identity for one immutable focus interval."""

    start = as_beijing(start_at).astimezone(timezone.utc).isoformat(timespec="microseconds")
    end = as_beijing(end_at).astimezone(timezone.utc).isoformat(timespec="microseconds")
    identity = "\x1f".join(
        (
            str(device_id or "").strip()[:120],
            str(session_id or "").strip()[:160],
            start,
            end,
        )
    )
    return f"fs2:{hashlib.sha256(identity.encode('utf-8')).hexdigest()}"
